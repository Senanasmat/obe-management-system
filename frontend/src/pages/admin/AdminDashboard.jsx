    ...
    return (
        ...
        >
            <Container fluid className="px-3 px-md-4">
                <div className="d-flex justify-content-between align-items-center mb-3 mt-3 pt-1 flex-wrap gap-2">
                    <div>
                        <h2 className="fw-bold text-dark mb-0" style={{ fontSize: 'clamp(1.3rem, 5vw, 1.8rem)' }}>SuperAdmin Dashboard</h2>

                    </div>
                    <div className="d-flex gap-2">
                        <Button variant="light" className="shadow-sm border-0 bg-white" style={{ fontSize: '0.8rem', padding: '0.5rem 1rem' }}>
                            <Calendar size={16} className="me-1" /> {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                        </Button>
                    </div>
                </div>

                <Row className="g-3 mb-4 pt-2">
                            ...
                            <Card className="shadow-sm border-0 overflow-hidden">
                                ...
                                <Card.Body className="p-3">
                                    <div className="d-flex flex-wrap gap-2">
                                        <Button className="rounded-pill px-3 py-2 fw-bold shadow-sm border-0 flex-grow-1" style={{ backgroundColor: '#4c1d95', fontSize: '0.9rem', minWidth: '140px' }} onClick={() => window.location.href = '/admin/students'}>
                                            <UserPlus size={16} className="me-2" /> Register Student
                                        </Button>
                                        <Button className="rounded-pill px-3 py-2 fw-bold shadow-sm border-0 flex-grow-1" style={{ backgroundColor: '#6d28d9', fontSize: '0.9rem', minWidth: '140px' }} onClick={() => window.location.href = '/admin/faculty'}>
                                            <UserPlus size={16} className="me-2" /> Register Faculty
                                        </Button>
                                        <Button variant="success" className="rounded-pill px-3 py-2 fw-bold shadow-sm flex-grow-1" style={{ fontSize: '0.9rem', minWidth: '140px' }} onClick={() => window.location.href = '/admin/courses'}>
                                            <Plus size={16} className="me-2" /> Add New Course
                                        </Button>
                                        <Button variant="secondary" className="rounded-pill px-3 py-2 fw-bold shadow-sm flex-grow-1" style={{ fontSize: '0.9rem', minWidth: '140px' }} onClick={() => window.location.href = '/admin/plos'}>
